  ...
  try {
    ...
    const worksheetData = [
      ...
      ...data.institutions.map((item, index) => [
        ...
        item.targetScale,
        item.actualScale,
        `${(item.scaleCompletionRate * 100).toFixed(2)}%`,
        `${(item.targetCustomerRatio * 100).toFixed(2)}%`,
        `${(item.actualCustomerRatio * 100).toFixed(2)}%`,
        `${(item.customerRatioCompletionRate * 100).toFixed(2)}%`,
        item.actualLeverage,
        `0.00%`,
        `${(item.actualRiskShare * 100).toFixed(2)}%`,
        `0.00%`,
        `${(item.actualCompensationRate * 100).toFixed(2)}%`,
        `${(item.actualRecoveryRate * 100).toFixed(2)}%`,
        item.policyScore,
        item.displayLabel,
      ]),
    ];
